...
def import_axfr_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                process_axfr_response(origin, nameserver,
                                      request.user, overwrite)
            except Exception as e:
                info_dict = {
                    'strerror': mark_safe(str(e)),
                }
...
def dynamic_ip_update_view(request):
    ...
    # If IP information is missing, the remote client's IP address will be
    # used.
    ...
    dyn_rrs = Record.objects.filter(
        domain=dyn_zone.domain, type__in=('A', 'AAAA'))
    if not dyn_rrs:
...
def zone_set_type_view(request, id_list):
    ...
    if request.method == 'POST':
        ...
        n += 1
        if n == 1:
            messages.info(request, "Successfully set the type of '%s' to '%s'" % (
                obj_display, domain_type))
        elif n > 1:
            messages.info(
                request, "Successfully set the type of %s zones to '%s'" % (n, domain_type))
        ...
        if form.is_valid():
            ...
            for n, zone_id in enumerate(id_list):
                ...
                if request.user != obj.created_by:
                    ...
                    # Find all resource records of this domain (excludes empty
                    # non-terminals)
                    qs = Record.objects.filter(
                        domain=obj).exclude(type__isnull=True)
                    # Now set the new TTL
                    for rr in qs:
                        rr.ttl = int(new_ttl)
                        ...
                        if reset_zone_minimum and rr.type == 'SOA':
                            ...
                            # SOA content:  primary hostmaster serial refresh
                            # retry expire default_ttl
                            bits[6] = str(new_ttl)
            ...
            n += 1
            if n == 1:
                messages.info(request, "Successfully updated %s resource records of '%s'" % (
                    record_count, obj_display))
            elif n > 1:
                messages.info(request, 'Successfully updated %s zones (%s total resource records).' % (
                    n, record_count))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user != domain_obj.created_by:
                messages.error(
                    request, "Insufficient permissions to clone domain '%s'" % force_unicode(domain_obj))
                return HttpResponseRedirect(reverse('admin:powerdns_manager_domain_changelist'))

            # Create the clone (Check for uniqueness takes place in
            # forms.ClonedZoneDomainForm
            ...
            # Find all resource records of this domain (also clones empty
            # non-terminals)
            ...
            for rr in domain_rr_qs:
                ...
                clone_rr_name = interchange_domain(
                    rr.name, domain_obj.name, clone_domain_name)
...
#                     )
            ...
            n += 1
            if n == 1:
                messages.info(request, "Successfully transfered domain '%s' to '%s'" % (
                    obj_display, owner_display))
            elif n > 1:
                messages.info(
                    request, 'Successfully transfered %d domains.' % n)
...
def template_create_zone_view(request, template_id):
    ...
    if request.method == 'POST':
        ...
        if request.user != template_obj.created_by:
            messages.error(
                request, 'Permission denied for template: %s' % template_obj_display)
            return HttpResponseRedirect(reverse('admin:powerdns_manager_zonetemplate_changelist'))
        else:
            ...
            messages.info(request, "Successfully created zone '%s' from template '%s'." % (
                origin, template_obj.name))
