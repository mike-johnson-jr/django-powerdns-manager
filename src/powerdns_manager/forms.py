...
class SoaRecordModelForm(BaseRecordModelForm):
    ...
    """
    primary = forms.CharField(max_length=96, initial='', required=True, label=_(
        'primary nameserver'), help_text="""The name of the name server that was the original or primary source of data for this zone.""")
    hostmaster = forms.CharField(max_length=64, initial='', required=True, label=_(
        'hostmaster mailbox'), help_text="""A name which specifies the mailbox of the person responsible for this zone. This should be specified in the mailbox-as-domain-name format where the `@' character is replaced with a dot. Example: hostmaster.domain.tld represents hostmaster@domain.tld""")
    serial = forms.IntegerField(min_value=1, initial=1, required=True, label=_('serial'), widget=forms.TextInput(
        attrs={'readonly': 'readonly'}), help_text="""The serial is generated automatically and is not user-editable. The serial is a "version number" for this zone. DNS servers that rely on AXFR for zone transfers use this to determine when updates have occurred. Popular values to use are the Unix timestamp or a date in the form YYYYMMDD.""")
    refresh = forms.IntegerField(min_value=300, initial=10800, required=True, label=_(
        'refresh'), help_text="""The number of seconds after which slave nameservers should check to see if this zone has been changed. If the zone's serial number has changed, the slave nameserver initiates a zone transfer. Example: 28800""")
    retry = forms.IntegerField(min_value=300, initial=3600, required=True, label=_(
        'retry'), help_text="""This specifies the number of seconds a slave nameserver should wait before retrying if it attmepts to transfer this zone but fails. Example: 7200""")
    expire = forms.IntegerField(min_value=300, initial=604800, required=True, label=_(
        'expire'), help_text="""If for expire seconds the primary server cannot be reached, all information about the zone is invalidated on the secondary servers (i.e., they are no longer authoritative for that zone). Example: 604800""")
    default_ttl = forms.IntegerField(min_value=300, initial=3600, required=True, label=_(
        'minimum TTL'), help_text="""The minimum TTL field that should be exported with any RR from this zone. If any RR in the database has a lower TTL, this TTL is sent instead. Example: 86400""")
    ...
    def clean_hostmaster(self):
        ...
        if hostmaster.find('@') != -1:
            raise forms.ValidationError(
                """This should be specified in the mailbox-as-domain-name format where the `@' character is replaced with a dot. Example: hostmaster.domain.tld represents hostmaster@domain.tld""")
...
class SrvRecordModelForm(BaseRecordModelForm):
    ...
    weight = forms.IntegerField(min_value=1, required=True, label=_('weight'), widget=forms.TextInput(
        attrs={'size': '3'}), help_text="""A relative weight for records with the same priority.""")
    port = forms.IntegerField(min_value=1, max_value=65535, required=True, label=_('port'), widget=forms.TextInput(
        attrs={'size': '5'}), help_text="""The TCP or UDP port on which the service is to be found.""")
    target = forms.CharField(max_length=128, required=True, label=_('target'), widget=forms.TextInput(attrs={
                             'size': '25'}), help_text="""The canonical hostname of the machine providing the service (no trailing dot).""")
...
class ARecordModelForm(BaseRecordModelForm):
    ...
    def clean_content(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                """Content should contain an IPv4 address""")
        else:
            return content
...
class AaaaRecordModelForm(BaseRecordModelForm):
    ...
    def clean_content(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                """Content should contain an IPv6 address""")
        else:
            return content
...
class ZoneImportForm(forms.Form):
    ...
    """
    origin = forms.CharField(max_length=128, initial='', required=False, label=_(
        'Origin'), help_text="""Enter the origin, otherwise make sure this information is available in your zone file either by the $ORIGIN directive or by using an FQDN in the name field of each record. (optional)""")
    zonetext = forms.CharField(widget=forms.Textarea, initial='', required=True, label=_(
        'Zone file'), help_text="""Paste the zone file text. (required)""")
    overwrite = forms.BooleanField(required=False, label=_(
        'Overwrite'), help_text="""If checked, existing zone will be replaced by this one. Proceed with caution.""")
...
class AxfrImportForm(forms.Form):
    ...
    """
    origin = forms.CharField(max_length=128, initial='', required=True, label=_(
        'Origin'), help_text="""Enter the domain name to import.""")
    nameserver = forms.GenericIPAddressField(protocol='both', required=True, label=_(
        'Nameserver IP'), help_text="""Enter the IP address (IPv4 or IPv6 of the nameserver that contains zone information.""")
    overwrite = forms.BooleanField(required=False, label=_(
        'Overwrite'), help_text="""If checked, existing zone will be replaced by this one. Proceed with caution.""")
...
class ZoneTypeSelectionForm(forms.Form):
    ...
    domaintype = forms.ChoiceField(choices=Domain.DOMAIN_TYPE_CHOICES, required=True, label=_(
        'Zone type'), help_text="""Select the zone type. Native refers to native SQL replication. Master/Slave refers to DNS server based zone transfers.""")


class TtlSelectionForm(forms.Form):
    """This form is used in intermediate page that sets the RR TTL in bulk."""
    new_ttl = forms.IntegerField(min_value=settings.PDNS_DEFAULT_RR_TTL, required=True, label=_(
        'New TTL'), help_text="""Enter the new Time-To-Live (TTL) in seconds.""")
    reset_zone_minimum = forms.BooleanField(required=False, label=_('Reset minimum TTL of the zones?'),
                                            help_text="""If checked, the minimum TTL of the selected zones will be reset to the new TTL value.""")


class ClonedZoneDomainForm(forms.Form):
    """This form is used in intermediate page that sets the name of the cloned zone."""
    clone_domain_name = forms.CharField(max_length=255, required=True, label=_(
        'Domain Name'), help_text="""Enter the domain name of the clone.""")
    option_clone_dynamic = forms.BooleanField(required=False, initial=True, label=_(
        'Clone dynamic setting'), help_text="""If checked and the original zone is marked as dynamic, then the clone will also be marked as dynamic as well and a new API key will be generated for it.""")
    option_clone_metadata = forms.BooleanField(required=False, initial=True, label=_(
        'Clone zone metadata'), help_text="""If checked, the metadata associated with the original zone will be cloned too.""")
    ...
    def clean_clone_domain_name(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                'A zone with this name already exists. Please enter a new domain name.')
...
class ZoneTransferForm(forms.Form):
    ...
    transfer_to_username = forms.CharField(max_length=255, required=True, label=_(
        'Username'), help_text="""Enter the username of the account to which the zone will be transfered.""")
    ...
    def clean_transfer_to_username(self):
        ...
        try:
            ...
            raise forms.ValidationError(
                'An account with this username was not found.')
...
class TemplateOriginForm(forms.Form):
    ...
    origin = forms.CharField(max_length=255, required=True, label=_(
        'origin'), help_text="""Enter the origin of the new zone.""")
