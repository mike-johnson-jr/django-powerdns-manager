...
class Domain(models.Model):
    ...
    name = models.CharField(max_length=255, unique=True, db_index=True, verbose_name=_(
        'name'), help_text="""This field is the actual domain name. This is the field that powerDNS matches when it gets a request. The domainname should be in the format of: domainname.TLD (no trailing dot)""")
    master = models.CharField(max_length=128, blank=True, null=True, verbose_name=_(
        'master'), help_text="""Enter a comma delimited list of nameservers that are master for this domain. This setting applies only to slave zones.""")
    last_check = models.PositiveIntegerField(null=True, verbose_name=_(
        'last check'), help_text="""Last time this domain was checked for freshness.""")
    type = models.CharField(max_length=6, choices=DOMAIN_TYPE_CHOICES, default=settings.PDNS_DEFAULT_ZONE_TYPE, verbose_name=_(
        'type'), help_text="""Select the zone type. Native refers to native SQL replication. Master/Slave refers to DNS server based zone transfers.""")
    notified_serial = models.PositiveIntegerField(null=True, verbose_name=_(
        'notified serial'), help_text="""The last notified serial of a master domain. This is updated from the SOA record of the domain.""")
    account = models.CharField(max_length=40, blank=True, null=True, verbose_name=_(
        'account'), help_text="""Determine if a certain host is a supermaster for a certain domain name. (???)""")
...
#        return super(Domain, self).save(*args, **kwargs)
    ...
    def export_zone_html_link(self):
        html_link = '<a href="%s"><strong>export zone</strong></a>' % reverse(
            'export_zone', kwargs={'origin': self.name})
        return html_link
...
class Record(models.Model):
    ...
    type = models.CharField(max_length=10, null=True, db_index=True, choices=RECORD_TYPE_CHOICES, verbose_name=_(
        'type'), help_text="""Select the type of the resource record.""")
    content = models.CharField(max_length=64000, null=True, verbose_name=_(
        'content'), help_text="""This is the actual content of a Resource Record. For instance, in case of an A record, this is the IP address.""")
    ttl = models.PositiveIntegerField(blank=True, null=True, verbose_name=_(
        'TTL'), help_text="""The time in seconds the DNS-clients are allowed to cache this record. Also known as Time To Live (TTL).""")
    prio = models.PositiveIntegerField(null=True, verbose_name=_(
        'priority'), help_text="""For MX records, this should be the priority of the mail exchanger specified.""")
    disabled = models.BooleanField(default=False, verbose_name=_(
        'disabled'), help_text="""Enable or disable this Resource Record.""")
    ...
    ordername = models.CharField(max_length=255, null=True, db_index=True, verbose_name=_(
        'ordername'), help_text="""http://doc.powerdns.com/dnssec-modes.html#dnssec-direct-database""")

    # This is set to the current timestamp on every save
    change_date = models.PositiveIntegerField(null=True, verbose_name=_(
        'change date'), help_text="""Timestamp for the last update. This is used by PowerDNS internally.""")
...
class SuperMaster(models.Model):
    ...
    ip = models.GenericIPAddressField(unique=True, verbose_name=_(
        'IP address'), help_text="""IP address for supermaster (IPv4 or IPv6).""")
    # TODO: added ``unique=True`` to ``nameserver`` because it is set as
    # primary key
    nameserver = models.CharField(max_length=255, unique=True, verbose_name=_(
        'nameserver'), help_text="""Hostname of the supermaster.""")
    account = models.CharField(max_length=40, blank=True, null=True, verbose_name=_(
        'account'), help_text="""Account name (???)""")
...
class Comment(models.Model):
    ...
    type = models.CharField(max_length=10, verbose_name=_(
        'type'), help_text="""Select the type of this comment.""")
    modified_at = models.PositiveIntegerField(verbose_name=_(
        'modified at'), help_text="""Timestamp for the last modification time.""")
    account = models.CharField(max_length=40, verbose_name=_(
        'account'), help_text="""Account name (???)""")
    comment = models.CharField(max_length=64000, verbose_name=_(
        'comment'), help_text="""Comment body.""")
...
class DomainMetadata(models.Model):
    ...
    content = models.TextField(blank=True, null=True, verbose_name=_(
        'content'), help_text="""Enter the metadata.""")
...
class CryptoKey(models.Model):
    ...
    active = models.BooleanField(default=False, verbose_name=_(
        'active'), help_text="""Check to activate key.""")
    # TODO: Check if content may be empty
    content = models.TextField(blank=True, null=True, verbose_name=_(
        'content'), help_text="""Enter the key data.""")
...
class TsigKey(models.Model):
    ...
    name = models.CharField(max_length=255, verbose_name=_(
        'name'), help_text="""Enter a name for the key.""")
    algorithm = models.CharField(max_length=50, choices=ALGORITHM_CHOICES, verbose_name=_(
        'algorithm'), help_text="""Select hashing algorithm.""")
    secret = models.CharField(max_length=255, verbose_name=_(
        'secret'), help_text="""Enter the shared secret.""")
...
class DynamicZone(models.Model):
    ...
    api_key = models.CharField(max_length=64, null=True, verbose_name=_(
        'API Key'), help_text="""The API key is generated automatically. To reset it, use the relevant action in the changelist view.""")
    date_modified = models.DateTimeField(
        auto_now=True, verbose_name=_('Last Modified'))
...
class ZoneTemplate(models.Model):
    ...
    name = models.CharField(max_length=100, verbose_name=_(
        'name'), help_text="""Enter a name for the template.""")
    content = models.TextField(blank=True, null=True, verbose_name=_(
        'content'), help_text="""Enter the template content. The placeholder #origin# is expanded to the origin of the zone to which the template is applied.""")
    notes = models.TextField(blank=True, null=True, verbose_name=_(
        'notes'), help_text="""Space for notes about the template.""")
