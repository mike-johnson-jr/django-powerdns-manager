        ...
        try:
            ...
            if serial_date == curdate:
                if int(nn) == 99:
                    ...
                    # Is always 2 digits long
                    nn_new = str(int(nn) + 1).zfill(2)[:2]
...
def process_axfr_response(origin, nameserver, owner, overwrite=False):
    ...
    axfr_query = dns.query.xfr(
        nameserver, origin, timeout=5, relativize=False, lifetime=10)
    ...
    *****
    ...
    try:
        ...
        if overwrite:
            ...
            raise Exception(
                'Zone already exists. Consider using the "overwrite" option')
    ...
    for name, node in zone.nodes.items():
        ...
        for rdataset in rdatasets:
            ...
            for rdata in rdataset:
                ...
                rr = Record(
                    domain=the_domain,
                    # name is the dnspython node name
                ...
                if rdataset.rdtype == dns.rdatatype._by_text['SOA']:
                    ...
                    rr.type = 'SRV'
                    rr.content = '%d %d %s' % (
                        rdata.weight, rdata.port, str(rdata.target).rstrip('.'))
...
def generate_zone_file(origin):
    ...
    for rr in the_rrs:
        ...
        if rr.type == 'SOA':
            ...
            # SOA content:  primary hostmaster serial refresh retry expire
            # default_ttl
            ...
            rdataset = zone.find_rdataset(
                record_name, rdtype=rdtype, create=True)
...
def rectify_zone(origin):
    ...
    * Delegation: When the authoritative name server for a domain receives
    ...
    try:
        ...
        for rr in zone_rr_list:
            ...
            if rr.name in delegated_names_list:
                ...
                # Set ordername=NULL for A & AAAA records of delegated names
                # (glue)
        ...
        try:
            ...
            for rr in zone_rr_list:
                if rr.auth:
                    rr.ordername = pdnssec_hash_zone_record(
                        rr.domain.name, rr.name)
                else:
                    rr.ordername = None
        else:
            # NSEC3 'Narrow' Mode
    ...
    # List to store the hostnames for which an empty non-terminal will be
    # created.
    rr_terminal_todo = []
    ...
    for rr in zone_rr_list:
        ...
        for part in ordername_parts:
            ...
            # Add the hostname to the list of empty non-terminal too be
            # created.
...
def pdnssec_hash_zone_record(zone_name, record_name):
   ...
   The initial contents of this registry are:
    ...
    nsec3param = DomainMetadata.objects.get(
        domain=the_domain, kind='NSEC3PARAM')
